...
var centralModalLGInfoDemo =
  document.querySelector("#centralModalLGInfoDemo") || null;
if (!centralModalLGInfoDemo) {
  $("#addressbook").addClass("d-none");
} else {
  $("#addressbook").removeClass("d-none");
}
...
Userinfo = () => {
  $.get("/Userinfo", data => {
    ...
    $("#usercardimage").html(
      `<img src="/storage/Userimage/${user.image}" class="rounded-circle" alt="${user.image}">`
    );
  });
};
  ...
  $.get("/todo", data => {
    ...
    if (todo.count == 0) {
      ...
      todo.forEach(n => {
        ...
        todobody += ` <a href="#" class="list-group-item d-flex task justify-content-between dark-grey-text ${_class}" id="task${n.id}">${n.todo}
      <i class="fas fa-trash ml-auto todo text-danger" data-toggle="tooltip" data-placement="top" title="Click to delete" id="todo${n.id}"></i></a>`;
      });
    }
